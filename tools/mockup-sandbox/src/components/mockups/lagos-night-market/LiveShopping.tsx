...
export default function LiveShopping() {
  ...
  return (
    <div className={`w-[390px] h-[844px] relative overflow-hidden font-sans select-none ${isDark ? 'bg-[#050505] text-white' : 'bg-[#fbeed3] text-stone-900'}`}>
      {/* Video Background */}
      ...
      <div className="absolute top-12 left-4 right-4 flex items-center justify-between z-10">
        <div className={`flex items-center backdrop-blur-md rounded-full p-1 pr-3 border ${isDark ? 'bg-black/40 border-white/10' : 'bg-[#fff5d8]/75 border-stone-400/55'}`}>
          ...
          <div className="ml-2 flex flex-col">
            ...
            <span className={`text-[10px] leading-tight ${isDark ? 'text-white/70' : 'text-stone-700'}`}>2.4K watching</span>
          </div>
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`backdrop-blur-md h-10 w-10 rounded-full flex items-center justify-center border transition-colors ${isDark ? 'bg-black/40 border-white/10 hover:bg-white/20 text-white' : 'bg-[#fff5d8]/75 border-stone-400/55 hover:bg-stone-300/55 text-stone-900'}`}
          >
          ...
          <div className={`backdrop-blur-md h-10 w-10 rounded-full flex items-center justify-center border ${isDark ? 'bg-black/40 border-white/10 text-white' : 'bg-[#fff5d8]/75 border-stone-400/55 text-stone-900'}`}>
            <X className="h-5 w-5" />
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 pb-8 flex flex-col justify-end z-10">
        ...
        <div className="w-[70%] h-48 overflow-y-auto flex flex-col justify-end space-y-3 mb-4 mask-image-to-top">
          <div className="flex items-start gap-2">
            <Avatar className={`h-6 w-6 border ${isDark ? 'border-white/20' : 'border-stone-400/55'}`}>
            ...
            <div className={`backdrop-blur-sm rounded-xl rounded-tl-none p-2 border ${isDark ? 'bg-black/30 border-white/5' : 'bg-[#fff5d8]/75 border-stone-400/35'}`}>
              <p className={`text-[10px] font-bold mb-0.5 ${isDark ? 'text-white/50' : 'text-stone-500'}`}>Tunde</p>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className={`backdrop-blur-sm rounded-xl rounded-tl-none p-2 border ${isDark ? 'bg-black/30 border-white/5' : 'bg-[#fff5d8]/75 border-stone-400/35'}`}>
              <p className={`text-[10px] font-bold mb-0.5 ${isDark ? 'text-white/50' : 'text-stone-500'}`}>Femi</p>
              <p className="text-xs">Na which size you wear?</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Avatar className={`h-6 w-6 border ${isDark ? 'border-white/20' : 'border-stone-400/55'}`}>
              <AvatarFallback className={`text-[10px] ${isDark ? 'bg-white/20 text-white' : 'bg-stone-300/45 text-stone-900'}`}>O</AvatarFallback>
            </Avatar>
            <div className={`backdrop-blur-sm rounded-xl rounded-tl-none p-2 border ${isDark ? 'bg-black/30 border-white/5' : 'bg-[#fff5d8]/75 border-stone-400/35'}`}>
        ...
        <div className="flex gap-3 items-end">
          {/* Pinned Product Card */}
          <div className={`flex-1 backdrop-blur-xl border rounded-2xl p-3 flex gap-3 relative overflow-hidden ${isDark ? 'bg-black/60 border-[#ff00ff]/30 shadow-[0_0_20px_rgba(255,0,255,0.15)]' : 'bg-[#fff5d8]/85 border-[#d900d9]/30 shadow-md'}`}>
            <div className={`absolute top-0 right-0 w-16 h-16 blur-xl rounded-full ${isDark ? 'bg-[#ff00ff]/20' : 'bg-[#d900d9]/10'}`}></div>
            <img src="/__mockup/images/lagos-product-serum.png" alt="Product" className={`w-16 h-16 rounded-xl object-cover border ${isDark ? 'border-white/10' : 'border-stone-400/35'}`} />
