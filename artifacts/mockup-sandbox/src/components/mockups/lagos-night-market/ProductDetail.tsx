  ...
  return (
    <div className={`w-[390px] h-[844px] relative overflow-hidden font-sans select-none flex flex-col ${isDark ? 'bg-[#050505] text-white' : 'bg-[#fcfcf9] text-zinc-900'}`}>
      ...
      <div className="absolute top-0 left-0 right-0 p-4 pt-12 flex justify-between z-20">
        <button className={`w-10 h-10 rounded-full backdrop-blur border flex items-center justify-center transition-colors ${isDark ? 'bg-black/40 border-white/10 hover:bg-black/60 text-white' : 'bg-white/60 border-black/10 hover:bg-white/80 text-zinc-900'}`}>
          <ChevronLeft className="h-6 w-6" />
        </button>
        <div className="flex gap-2">
          <button 
            onClick={() => setTheme(isDark ? "light" : "dark")}
            className={`w-10 h-10 rounded-full backdrop-blur border flex items-center justify-center transition-colors ${isDark ? 'bg-black/40 border-white/10 hover:bg-black/60 text-white' : 'bg-white/60 border-black/10 hover:bg-white/80 text-zinc-900'}`}
          >
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        {/* Image Carousel */}
        <div className={`relative w-full aspect-[4/5] ${isDark ? 'bg-[#111]' : 'bg-zinc-100'}`}>
          <img src="/__mockup/images/lagos-product-carousel-1.png" alt="Product" className="w-full h-full object-cover" />
          <div className={`absolute bottom-4 right-4 backdrop-blur text-[10px] font-bold px-2 py-1 rounded border ${isDark ? 'bg-black/60 text-white border-white/10' : 'bg-white/60 text-zinc-900 border-black/10'}`}>
            1 / 4
          </div>
        ...
        <div className="px-4 py-2">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className={`text-sm line-through ${isDark ? 'text-white/40' : 'text-zinc-400'}`}>₦32,000</p>
            </div>
          ...
          <div className={`flex items-center gap-3 mt-3 text-xs ${isDark ? 'text-white/60' : 'text-zinc-500'}`}>
            <div className="flex items-center gap-1">
              <Star className={`w-3 h-3 fill-current ${isDark ? 'text-[#ff00ff]' : 'text-[#d900d9]'}`} />
              <span className={`font-bold ${isDark ? 'text-white' : 'text-zinc-800'}`}>4.8</span>
              <span>(124 sold)</span>
            </div>
            <span>•</span>
        ...
        {/* Variants */}
        <div className={`px-4 py-4 mt-2 border-y ${isDark ? 'border-white/10 bg-white/5' : 'border-black/5 bg-black/5'}`}>
              ...
              <button 
                ...
                className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold border transition-all ${
                  ...
                  : isDark
                    ...
                    : "border-black/10 bg-white text-zinc-600 hover:border-black/30"
                }`}
              >
                {size}
              </button>
        ...
        <div className="px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs ${isDark ? 'text-white/50' : 'text-zinc-500'}`}>Lagos seller • 98% positive</p>
            </div>
          </div>
        ...
        {/* Delivery Options */}
        <div className={`px-4 py-4 mt-2 border-y ${isDark ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/5'}`}>
          ...
          <div className="space-y-3">
            <div className={`flex gap-3 p-3 rounded-xl border relative overflow-hidden ${isDark ? 'bg-black border-[#00ffff]/30' : 'bg-white border-[#00b3b3]/30'}`}>
              ...
              <div className="flex-1">
                ...
                <p className={`text-xs mt-1 ${isDark ? 'text-white/60' : 'text-zinc-500'}`}>Pick up from smart locker near you. Arrives in 1-2 days.</p>
              </div>
            </div>

            <div className={`flex gap-3 p-3 rounded-xl border ${isDark ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>
              <MapPin className={`w-5 h-5 mt-0.5 shrink-0 ${isDark ? 'text-white/50' : 'text-zinc-400'}`} />
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <p className={`text-sm font-bold ${isDark ? 'text-white/80' : 'text-zinc-800'}`}>PUDO Pickup Partner</p>
                  <p className={`text-sm font-bold ${isDark ? 'text-white/80' : 'text-zinc-800'}`}>₦500</p>
                </div>
                <p className={`text-xs mt-1 ${isDark ? 'text-white/50' : 'text-zinc-500'}`}>Pick up from a verified local shop.</p>
              </div>
            </div>

            <div className={`flex gap-3 p-3 rounded-xl border ${isDark ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>
              <Truck className={`w-5 h-5 mt-0.5 shrink-0 ${isDark ? 'text-white/50' : 'text-zinc-400'}`} />
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <p className={`text-sm font-bold ${isDark ? 'text-white/80' : 'text-zinc-800'}`}>Home Delivery</p>
                  <p className={`text-sm font-bold ${isDark ? 'text-white/80' : 'text-zinc-800'}`}>₦2,500</p>
                </div>
                <p className={`text-xs mt-1 ${isDark ? 'text-white/50' : 'text-zinc-500'}`}>Via Glovo / GIG. Arrives today by 6PM.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Protection */}
        <div className={`px-4 py-6 flex items-center gap-2 text-xs ${isDark ? 'text-white/60' : 'text-zinc-500'}`}>
      ...
      {/* Sticky Bottom CTA */}
      <div className={`absolute bottom-0 left-0 right-0 p-4 backdrop-blur-xl border-t flex gap-3 z-20 ${isDark ? 'bg-[#050505]/90 border-white/10' : 'bg-[#fcfcf9]/90 border-black/10'}`}>
        <button className={`flex-1 h-14 rounded-xl border font-bold transition-colors ${isDark ? 'bg-white/5 border-white/20 text-white hover:bg-white/10' : 'bg-black/5 border-black/10 text-zinc-900 hover:bg-black/10'}`}>
          Add to Cart
        </button>
