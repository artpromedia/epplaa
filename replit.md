...
- **Mobile App (Expo) — Auth Screens:** The Expo app at `apps/mobile` mirrors the web "Lagos Sunset" palette in `constants/colors.ts`. The auth flow lives under `app/(auth)/` (`sign-in`, `sign-up`, `phone`, `verify`) with a shared `AuthScaffold` (branded header, back button, keyboard-aware scroll), `AuthField`, and `AuthButton`. Welcome lives at `app/index.tsx` with three feature cards and CTAs into the auth stack. All submit handlers are UI-only stubs marked `TODO(auth)` referencing the matching web flow at `apps/web-buyer-spa/src/pages/auth/*` — Clerk's mobile SDK and the existing `startOtp/verifyOtp` API hooks need to be wired in a follow-up. Avoid `<Link asChild>` wrapping `<Text>` with array styles on web — it crashes react-native-web's style setter; use `<Pressable>` + `router.push` instead.
- **API Rate Limiting (Redis TODO):** `services/api-monolith/src/middlewares/apiRateLimit.ts` uses an in-memory token-bucket store. A `TODO(deploy)` comment marks where to swap in a Redis-backed store (Upstash/Memorystore via `REDIS_URL`) so limits shard across replicas — not yet provisioned. When the rate-limit store is degraded, the shopper app, manufacturer portal, and mobile app each render a small dismissible "Some actions may be slower than usual" banner. All three surfaces read from a single shared hook (`useRateLimitStoreBanner` in `packages/api-client-react`) which polls `/healthz` and resets dismissal whenever a fresh degradation incident begins.
- **Multi-instance Socket.IO (Task #21):** `services/api-monolith/src/lib/socket.ts` wires `@socket.io/redis-adapter` when `REDIS_URL` is set so chat/reactions/presence broadcasts cross every API replica and one chat room is shared cluster-wide. Per-room viewer counts use `getRoomSize` (delegates to `Namespace.in(room).fetchSockets()`) instead of the local-only `adapter.rooms` map, so adding a second API instance no longer splits the viewer count or chat. Without `REDIS_URL` the boot logs `socket_io_adapter_in_memory_single_instance_only` and falls back to the in-process adapter (single-instance dev only).
- **GitHub mirror sync (Task #274):** The Replit workspace is the source of truth; `https://github.com/artpromedia/epplaa.git` is a one-way backup mirror. `scripts/src/syncGithubMirror.ts` (npm script `pnpm --filter @workspace/scripts run sync-github-mirror`) compares local `refs/heads/main` to the remote via `git ls-remote`, no-ops if equal, otherwise pushes using HTTPS basic auth with `GITHUB_TOKEN` (URL-encoded into the userinfo so token-shape changes don't break URL parsing). All failures exit non-zero with a clear, token-redacted error. The script is wired as the **first** step of `scripts/post-merge.sh` (before `pnpm install` and the DB migration) so a failure later in the post-merge — install, migration, etc. — can't strand the mirror behind the workspace; every task merge into `main` triggers a push within seconds and a failed push surfaces as a failed post-merge step in the merge log. Override targets via `GITHUB_MIRROR_URL` / `GITHUB_MIRROR_BRANCH` env vars if the mirror ever moves.
