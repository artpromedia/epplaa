# #164 — Catch payment-flow regressions automatically on every change

**Sprint:** Sprint 5 — Payments & Payouts  
**Status:** Backlog  

## Problem Statement

Catch payment-flow regressions automatically on every change. This task tracks the work required to implement, test, and deploy this capability as described in the sprint plan.

## Acceptance Criteria

- [ ] Tests are written and pass in CI
- [ ] Edge cases are covered
- [ ] CI gate blocks merges on failure

## Relevant Files

- `services/api-monolith/src/` — API server source
