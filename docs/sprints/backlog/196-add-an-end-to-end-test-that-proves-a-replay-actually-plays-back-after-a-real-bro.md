# #196 — Add an end-to-end test that proves a replay actually plays back after a real broadcast

**Sprint:** Sprint 8 — Live Streaming & Recordings  
**Status:** Backlog  

## Problem Statement

Add an end-to-end test that proves a replay actually plays back after a real broadcast. This task tracks the work required to implement, test, and deploy this capability as described in the sprint plan.

## Acceptance Criteria

- [ ] Tests are written and pass in CI
- [ ] Edge cases are covered
- [ ] CI gate blocks merges on failure

## Relevant Files

- `artifacts/api-server/src/lib/audit.ts` — audit logging
