# #166 — Surface the slowdown banner on the admin console too

**Sprint:** Sprint 10 — Admin Console & Moderation  
**Status:** Implemented  

## Problem Statement

Surface the slowdown banner on the admin console too. This task tracks the work required to implement, test, and deploy this capability as described in the sprint plan.

## Acceptance Criteria

- [ ] Feature is visible in the admin console UI
- [ ] Appropriate permission checks are enforced
- [ ] UI renders correctly in the existing test suite

## Relevant Files

- `apps/admin/src/` — admin console React app
