...
| `:white_check_mark: page RESOLVED` post in the same thread/channel | A subsequent iteration in the *same* workflow run returned to exit 0 — the probe sees the subsystem as healthy again. This is per-run only; cross-run recovery (this run all-green after a previous run paged) is intentionally silent and Sentry's auto-resolve is the source of truth across runs. The per-run-only contract is rehearsed weekly by the chat-firing job's `Rehearse cross-run silence ...` step (run A pages, run B is all-green, asserts run B's `NOTIFY_LOG_FILE` is empty) — see the `Rehearse cross-run silence ...` row in *Interpreting a failed rehearsal run* below for what a regression in that contract looks like. | Cross-check `/healthz` and the Sentry issue before fully closing the incident — a subsystem can flap healthy briefly and re-degrade. |
...
| Subsystem coverage | Every subsystem exposed by `/healthz`'s `subsystems` map that has a per-subsystem Sentry pager wired up is rehearsed on the same weekly schedule as a separate matrix entry: `rateLimitStore` (fingerprint `rate_limit_store_stuck_degraded_rehearsal`, alert tag `rate_limit_store_stuck_degraded`, subsystem tag `rate_limit`), `db` (fingerprint `stuck_degraded_rehearsal_db`, alert tag `db_stuck_degraded`, subsystem tag `db`), `paymentGatewayPaystack` (fingerprint `payment_gateway_paystack_stuck_degraded_rehearsal`, alert tag `payment_gateway_paystack_stuck_degraded`, subsystem tag `payment_gateway_paystack`), and `paymentGatewayFlutterwave` (fingerprint `payment_gateway_flutterwave_stuck_degraded_rehearsal`, alert tag `payment_gateway_flutterwave_stuck_degraded`, subsystem tag `payment_gateway_flutterwave`). The `paymentGateway<Name>` subsystem keys mirror `paymentGatewaySubsystemKey` in `lib/subsystemHealth.ts` and the matching watcher must already be registered on the staging api-server (which only happens when the gateway's secret — `PAYSTACK_SECRET_KEY` / `FLUTTERWAVE_SECRET_KEY` — is configured); an unregistered gateway returns HTTP 400 `unregistered_payment_gateway` from the inject step rather than silently no-op'ing, so the matrix entry fails loudly and on-call sees the misconfiguration in the Sunday workflow run instead of during a real outage. Each entry asserts the same inject -> probe -> verify -> clear cycle independently against its own subsystem, with its own per-subsystem fingerprint, so a regression in one subsystem's pager wiring (Sentry rule disabled, fingerprint renamed, on-call routing drifted, probe's per-subsystem JSON shape regressed) can't be masked by the others still working. The matrix entries are serialised (`max-parallel: 1`) so each one observes a clean staging state — the probe's "worst offender" pick would otherwise flake non-deterministically when multiple subsystems are stuck simultaneously. `fail-fast: false` means a `db` (or per-gateway) regression doesn't suppress the `rateLimitStore` (or other gateway) signal. **When you add a new real gateway to `lib/payments.ts`, mirror it as a new matrix entry in `rehearse-healthz-degraded.yml` AND extend this row, otherwise the new gateway's pager wiring will not be rehearsed weekly.** When you remove a gateway (or its secret on staging), remove the matching matrix entry first or every Sunday's run will fail on the deliberately-unregistered watcher. |
| Synthetic injector | The api-server exposes guarded `/api/_rehearsal/inject-stuck-degraded` and `/api/_rehearsal/clear-stuck-degraded` endpoints that flip the watcher named by the request body's `subsystem` field into a synthetic `degraded` state with a caller-supplied `firstFailureAt`. Accepted values: the fixed set `rateLimitStore` / `db` / `auditChain` / `auditDlq`, plus any dynamic `paymentGateway<Name>` key (e.g. `paymentGatewayPaystack`) matching `^paymentGateway[A-Z][A-Za-z0-9]*$` — the regex enforces the same camelCase spelling produced by `paymentGatewaySubsystemKey` in `lib/subsystemHealth.ts`, so a typo like `paymentgatewayPaystack` (lowercase `g`) is rejected at the parser with `invalid_subsystem` rather than silently routed to the wrong watcher. For `paymentGateway<Name>`, the endpoint feeds the watcher registered at api-server boot by `lib/payments.ts` for the matching gateway and returns HTTP 400 `unregistered_payment_gateway` if no watcher is registered — it deliberately does NOT auto-register a watcher on demand, because the dev-mock fallback gateway is intentionally unregistered (a permanently-healthy `paymentGatewayDevmock` /healthz entry must never appear). Source: `artifacts/api-server/src/routes/healthzRehearsal.ts`. The endpoints return 404 unless `HEALTHZ_REHEARSAL_ENABLED=1` is set on the staging api-server, and additionally require an `X-Rehearsal-Token` header that timing-safely matches `HEALTHZ_REHEARSAL_TOKEN`. **Enable both env vars on staging only - never production.** |
...
| `Verify Sentry ingested the rehearsal events` | (a) Sentry is dropping/scrubbing events - check the Sentry stats page for the rehearsal project. (b) The `event:read` scope is missing on the auth token. (c) **Fingerprint dedup broken**: if the failure message says "expected ... 1 issue, saw 5", the per-subsystem fingerprint stopped collapsing iterations - verify nothing renamed `rate_limit_store_stuck_degraded_rehearsal` (rateLimitStore matrix entry), `stuck_degraded_rehearsal_db` (db matrix entry), `payment_gateway_paystack_stuck_degraded_rehearsal` (paymentGatewayPaystack matrix entry), or `payment_gateway_flutterwave_stuck_degraded_rehearsal` (paymentGatewayFlutterwave matrix entry), and that no new Sentry inbound filter or processor is forcing per-event grouping. The failing matrix entry's job name names the subsystem so you know which fingerprint to check. (d) **Tag stripped**: if the failure says "ingested event tags mismatched", Sentry's PII scrubber or a new `Tag Filter` rule is dropping `alert` / `subsystem` / `rehearsal` — or the expected per-subsystem tag values for the failing matrix entry have drifted from the production pager rule. The on-call page body for production would also be missing those tags. (e) **probe_output stripped**: if the failure says "event ... has no probe_output extra", Sentry's PII scrubber or a project rule is stripping the `--extra probe_output` payload. The on-call page body is composed FROM that extra (the probe JSON line with subsystem / streak duration / threshold / firstFailureAt), so production pages would arrive with no actionable detail. | Sentry project settings -> Data Scrubbing AND Alerts -> Issue alerts in the rehearsal project. The matched event/issue ID is surfaced in the workflow run summary so you can open the offending event directly. |
| `Clear synthetic streak` | Staging is unreachable. Manually POST `clear-stuck-degraded` with the subsystem named in the failing matrix entry's job (`rateLimitStore`, `db`, `paymentGatewayPaystack`, or `paymentGatewayFlutterwave`) before leaving — otherwise the per-minute probe workflow against staging will start paging for real for that subsystem. The `if: always()` guard means this step runs even if everything before it failed; if **this** step is the failure, the synthetic streak for THIS matrix entry's subsystem is still live on staging. **Special case for paymentGateway entries**: if the failure message says `unregistered_payment_gateway`, the inject step itself never registered a streak (the staging deploy doesn't have the gateway's secret configured, so `lib/payments.ts` skipped registering the watcher). In that case there's nothing to clear — the right fix is to either configure the gateway secret on staging (so the watcher gets registered at the next boot), or to remove the matching matrix entry from `rehearse-healthz-degraded.yml` so the workflow stops failing every Sunday on a deliberately-unregistered gateway. | Re-run with the same token from your shell, e.g. `curl -X POST .../clear-stuck-degraded -H "X-Rehearsal-Token: $TOKEN" --data '{"subsystem":"paymentGatewayPaystack"}'`. |
...
| `Rehearse cross-run silence (a fresh all-green run after a paging run MUST stay silent in chat)` (in the `rehearse-chat-firing` job) | The production probe's per-run-only chat dedup contract has regressed. The previous step (`Run production probe loop ...`) covers FIRING+RESOLVED *within one run*; this step covers what happens *between runs*. The runbook's `:white_check_mark: page RESOLVED` row in *Interpreting the chat heads-up* documents that cross-run recovery is intentionally silent — Sentry's auto-resolve owns it across runs, and a chat RESOLVED on every all-green run after a real outage cleared would post one spurious recovery message every minute (per probe iteration after recovery) until someone reverts. The step's specific failure modes: (a) **"run B posted N chat line(s) on an all-green run"** — the contract broke. Most likely cause: a recent change to [`.github/scripts/healthz-probe-loop.sh`](../../.github/scripts/healthz-probe-loop.sh) added a state file, a workflow cache, or a `/tmp` persistence layer for the `notified_firing` / `notified_resolved` flags (e.g. someone tried to "dedup retries across runs" and inadvertently broke the per-run boundary). The shared script must keep those flags strictly in-process — initialised to 0 at the top of the script — so each new bash invocation observes a clean slate. (b) **"run A did not post a successful FIRING line"** — the precondition for the cross-run assertion failed: the synthetic streak didn't make it to the probe in run A, so run B has nothing to be "silent about". This is rarely a real cross-run regression — it's the same class of failure as the previous step's `no FIRING chat post observed`. (c) **"run B's probe loop exited N, expected 0"** — the between-runs `clear-stuck-degraded` POST didn't take, the staging api-server restarted between phases (the watcher is in-process), or staging is genuinely degraded. The empty-log check would test the wrong thing while the subsystem is still degraded, so the step bails first. (d) **"between-runs clear endpoint returned HTTP <non-200>"** — the rehearsal `clear-stuck-degraded` endpoint regressed or staging rotated the `HEALTHZ_REHEARSAL_TOKEN`. Same diagnosis as the matrix's `Inject synthetic stuck-degraded streak` row above (404 = kill switch off, 401 = token mismatch, 503 = server-side env missing). | (a): walk [`.github/scripts/healthz-probe-loop.sh`](../../.github/scripts/healthz-probe-loop.sh) — search for any new persistence to `notified_firing` / `notified_resolved` (filesystem reads/writes, env-var reads, `actions/cache` artifacts, etc.). The fix is to remove the cross-run state, NOT to update this assertion. (b): walk the `Run production probe loop ...` row in this same table — same root causes. (c): re-trigger the workflow via `workflow_dispatch:` once staging stabilises; if it stays red, hit `/api/_rehearsal/clear-stuck-degraded` from your shell with the staging token and the named subsystem. (d): repo Settings -> Secrets and variables -> Actions; verify `secrets.HEALTHZ_REHEARSAL_TOKEN` matches staging's `HEALTHZ_REHEARSAL_TOKEN` env var. The always-run backstop `Clear synthetic streak` step in this same job restores staging even when this assertion fails — but check the workflow log to confirm the backstop's HTTP status was 200 before walking away. |
